...
object Query2 {
  def main(args: Array[String]) {
    ...
    // Valid for Jena
    val records = allJoined.map(record => new Tuple3(record.getField0.getField0.getField1.getString(0),
      record.getField0.getField0.getField0.getField0.getString(0), record.getField0.getField1.getString(0)))
...
//      .filter(tuple3 => tuple3.field1 >= 4 && tuple3.field2 > 130)
//      .withName("Filter based on PageRank and DegreeCentrality value")

//      .map(tuple3 => tuple3.field2).withName("Get only projected variable ?s")
