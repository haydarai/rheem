 ...
 *
 */
public class RidgeRegressionLearner {
    double[][] longley = {
    ...
    public static List<doubleInstance> readDataSet(String file) throws FileNotFoundException {
        List<doubleInstance> dataset = new ArrayList<doubleInstance>();
        ...
        try {
            ...
            while(scanner.hasNextLine()) {
                ...
                doubleInstance instance = new doubleInstance(label, data);
    ...
    public void learnCosts() throws FileNotFoundException {
        ...
        // read data
        List<doubleInstance> instanceList = readDataSet(this.getClass().getResource("/planVectors").getFile());
        ...
        int index =0;
        for(doubleInstance x:instanceList){
            X[index]=x.getX();
            Y[index]=x.getLabel();
            index++;
        }
        ...
        rss = 0.0;
        for (int i = 0; i < n; i++) {
            trainx = Math.slice(longley, loocv.train[i]);
            trainy = Math.slice(y, loocv.train[i]);
            LASSO lasso = new LASSO(longley, y, 100.0);
